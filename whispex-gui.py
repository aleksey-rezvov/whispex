...
class SettingsDialog(QtWidgets.QDialog):
    def __init__(self, settings_manager, parent=None):
        ...
        default_prompt = settings_manager.get(
    ...
    def apply_settings(self):
        ...
        success1 = self.settings_manager.set(
            SettingsSection.WHISPER, WhisperSettings.TEMPERATURE, temperature
        )
        success2 = self.settings_manager.set(
            SettingsSection.WHISPER, WhisperSettings.PROMPT, prompt
        )
...
if __name__ == "__main__":
    ...
    notification_icon = (
        QtGui.QIcon(str(tray_icon.icon_path))
        if tray_icon.icon_path.exists()
        else QtGui.QIcon.fromTheme("audio-input-microphone")
    )

    tray_icon.showMessage(
        "Whispex", "Application running in system tray", notification_icon, 3000
    )
